...
SPI_Ard_SendCommand:
    ...
    lda #(SLOAD | SPI_DEV2_ARD | SCK)
    sta PORT3B
    lda #(SPI_SCK)
    sta PORT2A

    lda #(SLOAD | SPI_DEV2_ARD)
    sta PORT3B
    lda #0
    sta PORT2A
    ...
    ;lda #(SLOAD | OE | SCK| SPI_DEV2_ARD)
    ;sta PORT3B

    ;lda #(SLOAD | OE| SPI_DEV2_ARD)
    ;sta PORT3B

    ;lda #(SLOAD | OE | SCK | RCK_OUT| SPI_DEV2_ARD)
    ;sta PORT3B
