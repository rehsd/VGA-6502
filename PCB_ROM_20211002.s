;General Information
    ...
    ;	vasm6502_oldstyle.exe -Fbin -dotdir -wdc02 g7.s -o g7.bin
...
MOUSE_LEFT              = %00000001
MOUSE_UP                = %00000010
MOUSE_RIGHT             = %00000100
MOUSE_DOWN              = %00001000
MOUSE_CLICK_LEFT        = %00010000
MOUSE_CLICK_RIGHT       = %00100000
MOUSE_CLICK_MIDDLE      = %01000000
;VIA5 PORTA
MOUSE_LEFT_UP           = %00000001
MOUSE_RIGHT_UP          = %00000010
MOUSE_RIGHT_DOWN        = %00000100
MOUSE_LEFT_DOWN         = %00001000
...
reset:
  ...
  lda #%01111111	;Used to enable CB1 (counter for temperature sensor refresh) and CA1 (ACIA) , and disables everything else. Set/Clear Timer1 Timer2 CB1 CB2 ShiftRegister CA1 CA2 
  sta IER3
...
VIA5_CB1_handler3:
  ...
  sta $6A
  AND #MOUSE_LEFT_UP
  bne Handle_Mouse_Left_Up
  lda $6A
  AND #MOUSE_RIGHT_UP
  bne Handle_Mouse_Right_Up
  lda $6A
  AND #MOUSE_RIGHT_DOWN
  bne Handle_Mouse_Right_Down
  lda $6A
  AND #MOUSE_LEFT_DOWN
  bne Handle_Mouse_Left_Down
...
VIA1_IRQ:               ;keyboard
  ...
  jmp VIA2_IRQ_OUT
VIA5_CB1_handler:       ;USB mouse handler
  jmp VIA5_CB1_handler1
...
VIA3_CA1_handler:       ;ACIA
  ...
  recvTemp:
      ...
      CA1_handler_out:
          ...
          jmp irq_done
VIA2_IRQ:               ;joystick
  ;to do -- check interrupt source on VIA IER (CB1, CB2, CA1, CA2) - for now, assuming joystick is only possible interrupt source
  lda PORT2B
  sta $69
  AND #JOYSTICK_DOWN
...
Handle_Joystick_Up:
  ...
  jmp VIA2_IRQ_OUT
VIA5_CB1_handler1:
    lda PORT5B    ;clear interrupt
    sta $6A
    AND #MOUSE_LEFT
    bne Handle_Mouse_Left
    lda $6A
    AND #MOUSE_UP
    bne Handle_Mouse_Up
    lda $6A
    AND #MOUSE_RIGHT
    bne Handle_Mouse_Right
    lda $6A
    AND #MOUSE_DOWN
    bne Handle_Mouse_Down
    jmp VIA5_CB1_handler2
Handle_Mouse_Left:
    ;jsr print_dec_lcd
    jsr RestoreCurrentPixelInfo
    ldx fill_region_start_x
...
VIA5_CB1_handler2:
  lda $6A
  AND #MOUSE_CLICK_LEFT
  bne Handle_Mouse_Click_Left
  lda $6A
  AND #MOUSE_CLICK_MIDDLE
  bne Handle_Mouse_Click_Middle
  lda $6A
  AND #MOUSE_CLICK_RIGHT
  bne Handle_Mouse_Click_Right
  jmp VIA5_CB1_handler3
...
Handle_Mouse_Click_Right:
    ...
    jmp irq_done
VIA5_IRQ_OUT:
  ;lda #$E0  ;start counting up at this value; higher # = shorter delay; adjust based on preference for movement speed (and on clock speed)
  ;sta delayDurationHighByte
  ;jsr Delay
